...
class Generator(nn.Module):
    ...
    def __init__(self, z_dim, shared_dim, img_size, g_conv_dim, g_spectral_norm, attention, attention_after_nth_gen_block, activation_fn,
                 conditional_strategy, num_classes, initialize, G_depth, mixed_precision, feat_enc_w=None, feat_gen_w=None, rec_w=None):
        ...
        self.feat_enc_w = feat_enc_w 
        self.feat_gen_w = feat_gen_w 
        self.rec_w = rec_w

    def forward(self, z, label, shared_label=None, evaluation=False, feat_enc=None, use_feat=True, output_feat_gen=False):
        with torch.cuda.amp.autocast() if self.mixed_precision is True and evaluation is False else dummy_context_mgr() as mp:
            ...
            target_reso = 32
            if output_feat_gen and act.shape[-1]==target_reso: # original one is 32
                return act
            # summing two features
            if use_feat:
                act += self.feat_enc_w['4'] * feat_enc[4]
            counter = 0
            for index, blocklist in enumerate(self.blocks):
                for block in blocklist:
                    if isinstance(block, Self_Attn):
                        act = block(act)
                        if use_feat:
                            if act.shape[-1]  >= 4 and  32  >= act.shape[-1] :
                                act += self.feat_enc_w[act.shape[-1]] * feat_enc[act.shape[-1]] 
                    else:
                        ...
                        if use_feat:
                            if act.shape[-1]  >= 4 and  32  >= act.shape[-1] :
                                reso = '%s'%(act.shape[-1])
                                #act += self.feat_enc_w[reso]  * torch.cat([feat_enc[act.shape[-1]], feat_enc[act.shape[-1]]], 1)
                                act += self.feat_enc_w[reso] * feat_enc[act.shape[-1]] 
                        counter +=1
...
class Generator_Aux(nn.Module):
    """Generator."""
    def __init__(self, z_dim, shared_dim, img_size, g_conv_dim, g_spectral_norm, attention, attention_after_nth_gen_block, activation_fn,
                 conditional_strategy, num_classes, initialize, G_depth, mixed_precision, feat_enc_w=None, feat_gen_w=None, rec_w=None):
        super(Generator_Aux, self).__init__()
        g_in_dims_collection = {"32": [g_conv_dim*4, g_conv_dim*4, g_conv_dim*4],
                                "64": [g_conv_dim*16, g_conv_dim*8, g_conv_dim*4, g_conv_dim*2],
                                "128": [g_conv_dim*16, g_conv_dim*16, g_conv_dim*8, g_conv_dim*4, g_conv_dim*2],
                                "256": [g_conv_dim*16, g_conv_dim*16, g_conv_dim*8, g_conv_dim*8, g_conv_dim*4, g_conv_dim*2],
        ...
        self.feat_gen_w = feat_gen_w 
        self.rec_w = rec_w
        self.reso_index = {32:3, 16:2, 8:1, 4:0}

    def forward(self, z, label, shared_label=None, evaluation=False, feat_gen=None):
        with torch.cuda.amp.autocast() if self.mixed_precision is True and evaluation is False else dummy_context_mgr() as mp:
            zs = torch.split(z, self.chunk_size, 1)
            z = zs[0]
            if shared_label is None:
            ...
            for index, blocklist in enumerate(self.blocks):
                for block in blocklist:
                    if isinstance(block, Self_Attn):
                        if index >= self.reso_index[feat_gen.shape[-1]]:
                            act = block(act)
                    else:
                        if index >= self.reso_index[feat_gen.shape[-1]]:
                            act = block(act, labels[counter])
...
class Discriminator(nn.Module):
    ...
    def forward(self, x, label, evaluation=False, mode='dis', output_d_feat=False):
        with torch.cuda.amp.autocast() if self.mixed_precision is True and evaluation is False else dummy_context_mgr() as mp:
            h = x

            # save latent representaion 
            feat_enc = {}

            for index, blocklist in enumerate(self.blocks):
                for block in blocklist:
                    h = block(h)
                    if h.shape[-1]  >= 4 and  32  >= h.shape[-1] :
                        feat_enc[h.shape[-1]] = h.detach().clone() 
            if mode=='enc':
                return feat_enc
            h = self.activation(h)
            ...
            if self.conditional_strategy == 'no':
                ...
                both_out = proj + authen_output
                return (both_out, feat_enc) if output_d_feat else both_out
...
class Aligner(nn.Module):
    ...
    def __init__(self, img_size, d_conv_dim, d_spectral_norm, attention, attention_after_nth_dis_block, activation_fn, conditional_strategy,
        ...
        self.blocks = []
        for index in range(len(self.in_dims) -2): # we only consider [32, 16, 8, 4] if the input image is 128*128, ingnore the seconde block which performs for 4*4. Note there are two blocks for 64*64, one of which is the attention layer
            index += 2
            if index == 0:
                self.blocks += [[DiscBlock(in_channels=self.in_dims[index],
                                              out_channels=self.out_dims[index],
                ...
                self.blocks += [[DiscBlock(in_channels=self.in_dims[index],
                                           out_channels=self.out_dims[index],
                                           d_spectral_norm=False, #  : updating it into False
                                           activation_fn=activation_fn,
                                           downsample=down[index])]]
    ...
    def forward(self, x):
        with torch.cuda.amp.autocast() if self.mixed_precision is True and evaluation is False else dummy_context_mgr() as mp:
            ...
            for index, blocklist in enumerate(self.blocks):
                for block in blocklist:
                    resu_curr = resu_feat[index] 
                    feat_enc[resu_curr] = block(x[resu_curr]) 
                    if  resu_curr!=4:
                        assert feat_enc[resu_curr].shape[1] == 2*x[resu_curr].shape[1]   
                    else:
                        assert feat_enc[resu_curr].shape[1] == x[resu_curr].shape[1]   
            return feat_enc
