...
class make_worker(object):
    def __init__(self, cfgs, run_name, best_step, logger, writer, n_gpus, gen_aux_model, gen_model, enc_model, ali_model,  dis_model, inception_model, Gen_copy, Gen_aux_copy, 
                 Gen_ema, Gen_aux_ema, train_dataset, eval_dataset, train_dataloader, eval_dataloader, G_aux_optimizer, G_optimizer, A_optimizer, D_optimizer, G_loss, G_aux_loss,
        ...
        self.gen_aux_model = gen_aux_model
        self.enc_model = enc_model
        self.ali_model = ali_model
        self.dis_model = dis_model
        self.inception_model = inception_model
        self.Gen_copy = Gen_copy
        self.Gen_aux_copy = Gen_aux_copy
        self.Gen_ema = Gen_ema
        self.Gen_aux_ema = Gen_aux_ema
        ...
        self.target_classes = cfgs.target_classes
        ...
        self.G_aux_optimizer = G_aux_optimizer
        ...
        if self.dataset_name == "imagenet":
            self.cla_name = {149: 'animal', 256: 'food', 555:'bird'}
            self.test_name = {149: 1490, 256: 1280, 555:1280}
            self.num_eval = {'train':300, 'valid':300}
    ...
    def train(self, current_step, total_step):
        ...
        self.gen_aux_model.train()
        self.ali_model.train()
        if self.Gen_copy is not None:
            self.Gen_copy.train()
            self.Gen_aux_copy.train()
        if self.rank == 0: self.logger.info('Start training....')
        step_count = current_step
        train_iter = iter(self.train_dataloader)
        self.num_classes = self.target_classes # 
        self.gen_model.num_classes = self.target_classes # 
        if self.Gen_copy is not None:
            self.Gen_copy.num_classes = self.target_classes # 
            self.Gen_aux_copy.num_classes = self.target_classes # 
        ...
        while step_count <= total_step:
            ...
            toggle_grad(self.gen_aux_model, on=False, freeze_layers=-1)
            toggle_grad(self.enc_model, on=False, freeze_layers=-1) # the encoder is fixed
            toggle_grad(self.ali_model, on=False, freeze_layers=-1) # aligner 
            ...
            for step_index in range(self.d_steps_per_iter):
                ...
                for acml_index in range(self.accumulation_steps):
                    try:
                        ...
                        source_images, source_labels = next(train_iter)
                    ...
                    source_images, source_labels = source_images.to(self.rank), source_labels.to(self.rank)
                    with torch.cuda.amp.autocast() if self.mixed_precision else dummy_context_mgr() as mpc:
                        ...
                        # Encoder
                        feat_enc = self.enc_model(source_images, None, mode='enc')
                        feat_enc = self.ali_model(feat_enc)
                        fake_images = self.gen_model(zs, fake_labels, feat_enc=feat_enc)
                        # Feature from main generator, which is for the auxiliary generator
                        self.gen_model.eval()
                        feat_gen_32 = self.gen_model(zs, fake_labels, use_feat=False, output_feat_gen=True)
                        fake_images_aux = self.gen_aux_model(zs, fake_labels, feat_gen=feat_gen_32)
                        self.gen_model.train()
                        ...
                        if self.conditional_strategy == "ACGAN":
                            ...
                            dis_out_fake_aux = self.dis_model(fake_images_aux, fake_labels)
                        ...
                        dis_acml_loss +=  self.cfgs.aux_w*self.D_loss(dis_out_real, dis_out_fake_aux)# Auxiliary Generator
                        if self.conditional_strategy == "ACGAN":
            ...
            toggle_grad(self.gen_aux_model, True, freeze_layers=-1)
            toggle_grad(self.ali_model, True, freeze_layers=-1)
            for step_index in range(self.g_steps_per_iter):
                self.G_optimizer.zero_grad()
                self.G_aux_optimizer.zero_grad()
                ...
                for acml_step in range(self.accumulation_steps):
                    with torch.cuda.amp.autocast() if self.mixed_precision else dummy_context_mgr() as mpc:
                        ...
                        if self.conditional_strategy == "ACGAN":
                            ...
                            dis_out_fake, feat_fake = self.dis_model(fake_images, fake_labels, output_d_feat=True)
                            _, feat_source = self.dis_model(source_images, fake_labels, output_d_feat=True)
                            dis_out_fake_aux = self.dis_model(fake_images_aux, fake_labels, output_d_feat=False)
                        elif self.conditional_strategy in ["NT_Xent_GAN", "Proxy_NCA_GAN", "ContraGAN"]:
                        ...
                        gen_acml_loss += self.cfgs.aux_w*self.G_loss(dis_out_fake_aux)

                        # reconstu
                        rec_loss = Rec_loss(feat_source, feat_fake, self.gen_model.rec_w)
                        gen_acml_loss += rec_loss
                ...
                if self.mixed_precision: # False
                    ...
                    self.scaler.step(self.G_aux_optimizer)
                    self.scaler.update()
            ...
            if step_count % self.print_every == 0 and self.rank == 0:
                ...
                self.writer.add_scalars('Losses', {'discriminator': dis_acml_loss.item(),
                                                   'generator': gen_acml_loss.item(), 'rec_loss': rec_loss.item(),}, step_count)
                if self.ada:
            ...
            if step_count % self.save_every == 0 or step_count == total_step:
                if self.evaluate:
                    ...
                    self.save_images(False, False,  "N/A", png=True, npz=False, All_classes=True)#  :is_generate is False, since we perform I2I and need input source images.
                    is_best = self.evaluation(step_count, False, "N/A", condition_input=True)
    ...
    def save(self, step, is_best):
        ...
        self.enc_model.eval()
        self.gen_model.eval()
        self.gen_aux_model.eval()
        self.enc_model.eval()
        self.ali_model.eval()
        if self.Gen_copy is not None:
            self.Gen_copy.eval()
            self.Gen_aux_copy.eval()

        self.checkpoint_dir = os.path.join(self.cfgs.log_output_path, 'checkpoint')
        ...
        if isinstance(self.gen_model, DataParallel) or isinstance(self.gen_model, DistributedDataParallel):
            ...
            gen_aux = self.gen_aux_model.module
            enc, ali = self.enc_model.module, self.ali_model.module
            if self.Gen_copy is not None:
                gen_copy = self.Gen_copy.module
                gen_aux_copy = self.Gen_aux_copy.module
        else:
            gen, dis = self.gen_model, self.dis_model
            gen_aux = self.gen_aux_model
            enc, ali = self.enc_model, self.ali_model
            if self.Gen_copy is not None:
                gen_copy = self.Gen_copy
                gen_aux_copy = self.Gen_aux_copy
        ...
        g_aux_states = {'seed': self.seed, 'run_name': self.run_name, 'step': step, 'best_step': self.best_step,
                    'state_dict': gen_aux.state_dict(), 'optimizer': self.G_aux_optimizer.state_dict(), 'ada_p': self.ada_aug_p}

        a_states = {'seed': self.seed, 'run_name': self.run_name, 'step': step, 'best_step': self.best_step,
                    'state_dict': ali.state_dict(), 'optimizer': self.A_optimizer.state_dict(), 'ada_p': self.ada_aug_p}
        ...
        e_states = {'seed': self.seed, 'run_name': self.run_name, 'step': step, 'best_step': self.best_step,
                    'state_dict': enc.state_dict(), 'optimizer': self.D_optimizer.state_dict(), 'ada_p': self.ada_aug_p}
        ...
        if len(glob.glob(join(self.checkpoint_dir,"model=G-{when}-weights-step*.pth".format(when=when)))) >= 1:
            ...
            if  len(glob.glob(join(self.checkpoint_dir,"model=A-{when}-weights-step*.pth".format(when=when)))) > 0:
                find_and_remove(glob.glob(join(self.checkpoint_dir,"model=A-{when}-weights-step*.pth".format(when=when)))[0])
            if  len(glob.glob(join(self.checkpoint_dir,"model=G_aux-{when}-weights-step*.pth".format(when=when)))) > 0:
                find_and_remove(glob.glob(join(self.checkpoint_dir,"model=G_aux-{when}-weights-step*.pth".format(when=when)))[0])
            if  len(glob.glob(join(self.checkpoint_dir,"model=E-{when}-weights-step*.pth".format(when=when)))) > 0:
        ...
        g_aux_checkpoint_output_path = join(self.checkpoint_dir, "model=G_aux-{when}-weights-step={step}.pth".format(when=when, step=str(step)))
        a_checkpoint_output_path = join(self.checkpoint_dir, "model=A-{when}-weights-step={step}.pth".format(when=when, step=str(step)))
        ...
        torch.save(g_aux_states, g_aux_checkpoint_output_path)
        torch.save(a_states, a_checkpoint_output_path)
        ...
        if when == "best":
            if len(glob.glob(join(self.checkpoint_dir,"model=G-current-weights-step*.pth"))) >= 1:
                ...
                if len(glob.glob(join(self.checkpoint_dir,"model=A-current-weights-step*.pth"))) > 0:
                    find_and_remove(glob.glob(join(self.checkpoint_dir,"model=A-current-weights-step*.pth"))[0])
                if len(glob.glob(join(self.checkpoint_dir,"model=G_aux-current-weights-step*.pth"))) > 0:
                    find_and_remove(glob.glob(join(self.checkpoint_dir,"model=G_aux-current-weights-step*.pth"))[0])
                if len(glob.glob(join(self.checkpoint_dir,"model=E-current-weights-step*.pth"))) > 0:
            ...
            g_aux_checkpoint_output_path_ = join(self.checkpoint_dir, "model=G_aux-current-weights-step={step}.pth".format(step=str(step)))
            a_checkpoint_output_path_ = join(self.checkpoint_dir, "model=A-current-weights-step={step}.pth".format(step=str(step)))
            ...
            torch.save(g_aux_states, g_aux_checkpoint_output_path_)
            torch.save(a_states, a_checkpoint_output_path_)
        ...
        if self.Gen_copy is not None:
            ...
            if len(glob.glob(join(self.checkpoint_dir, "model=G_aux_ema-{when}-weights-step*.pth".format(when=when)))) >= 1:
                find_and_remove(glob.glob(join(self.checkpoint_dir, "model=G_aux_ema-{when}-weights-step*.pth".format(when=when)))[0])
            ...
            g_aux_ema_checkpoint_output_path = join(self.checkpoint_dir, "model=G_aux_ema-{when}-weights-step={step}.pth".format(when=when, step=str(step)))

            torch.save(g_ema_states, g_ema_checkpoint_output_path)
            torch.save(g_aux_ema_states, g_aux_ema_checkpoint_output_path)
            ...
            if when == "best":
                ...
                if len(glob.glob(join(self.checkpoint_dir,"model=G_aux_ema-current-weights-step*.pth".format(when=when)))) >= 1:
                    find_and_remove(glob.glob(join(self.checkpoint_dir,"model=G_aux_ema-current-weights-step*.pth".format(when=when)))[0])
                ...
                g_aux_ema_checkpoint_output_path_ = join(self.checkpoint_dir, "model=G_aux_ema-current-weights-step={step}.pth".format(when=when, step=str(step)))

                torch.save(g_ema_states, g_ema_checkpoint_output_path_)
                torch.save(g_aux_ema_states, g_aux_ema_checkpoint_output_path_)
    ...
    ################################################################################################################################
    def evaluation(self, step, standing_statistics, standing_step, condition_input=False):
        ...
        with torch.no_grad() if self.latent_op is False else dummy_context_mgr() as mpc:
            ...
            precision, recall, f_beta, f_beta_inv = calculate_f_beta_score(self.eval_dataloader, generator, self.dis_model, self.inception_model, self.num_eval[self.eval_type],
                                                                           ...
                                                                           self.latent_op_step4eval, self.latent_op_alpha, self.latent_op_beta, self.rank, self.logger, condition_input, self.enc_model, self.ali_model)

            PR_Curve = plot_pr_curve(precision, recall, self.run_name, self.logger, log_output_path=self.cfgs.log_output_path)
            ...
            kl_score, kl_std = calculate_incep_score(self.eval_dataloader, generator, self.dis_model, self.inception_model, self.num_eval[self.eval_type],
                                                     ...
                                                     self.latent_op_beta, num_split, self.rank, self.logger, condition_input, self.enc_model, self.ali_model)
            ...
            fid_score, self.m1, self.s1 = calculate_fid_score(self.eval_dataloader, generator, self.dis_model, self.inception_model, self.num_eval[self.eval_type],
                                                              self.truncated_factor, self.prior, self.latent_op, self.latent_op_step4eval, self.latent_op_alpha,
                                                              self.latent_op_beta, self.rank, self.logger, self.mu, self.sigma, self.run_name, condition_input, self.enc_model, self.ali_model)
            ...
            if self.conditional_strategy in ['ProjGAN', 'ContraGAN', 'Proxy_NCA_GAN']:
                if self.dataset_name == "cifar10":
                    ...
                    if self.num_classes > 10:
                        classes = torch.tensor(random.sample(range(0, self.num_classes), 10), dtype=torch.long).to(self.rank)
                    else:
                        classes = torch.tensor([c for c in range(self.num_classes)], dtype=torch.long).to(self.rank)
                ...
                sim_heatmap = plot_sim_heatmap(sim_p.detach().cpu().numpy(), labels, labels, self.run_name, self.logger, log_output_path=self.cfgs.log_output_path)
            ...
            if self.D_loss.__name__ != "loss_wgan_dis":
                real_train_acc, fake_acc = calculate_accuracy(self.train_dataloader, generator, self.dis_model, self.D_loss, self.num_eval[self.eval_type],
                                                              ...
                                                              self.latent_op_beta, self.rank, cr=self.cr, logger=self.logger, eval_generated_sample=True, condition_input=condition_input, encoder=self.enc_model, aligner=self.ali_model)
                ...
                if self.eval_type == 'train':
                    ...
                    real_eval_acc = calculate_accuracy(self.eval_dataloader, generator, self.dis_model, self.D_loss, self.num_eval[self.eval_type],
                                                       self.truncated_factor, self.prior, self.latent_op, self.latent_op_step, self.latent_op_alpha,
                                                       self. latent_op_beta, self.rank, cr=self.cr, logger=self.logger, eval_generated_sample=False,  condition_input=condition_input, encoder=self.enc_model, aligner=self.ali_model)
    ...
    ################################################################################################################################
    def save_images(self, is_generate, standing_statistics, standing_step, png=True, npz=True, All_classes=False):
        ...
        with torch.no_grad() if self.latent_op is False else dummy_context_mgr() as mpc:
            ...
            if png:
                ...
                if All_classes:
                    with open('class_to_index/%s/I128_imgs.pickle'%(self.cla_name[self.target_classes]), 'rb') as handle:
                        class_to_index = pickle.load(handle)
                        index_to_class={class_to_index[i]:i for i in class_to_index.keys()}

                    for cate in range(self.target_classes):
                        save_all_cate_images_png(self.run_name, self.eval_dataloader, self.test_name[self.target_classes], self.num_classes, self.enc_model, self.ali_model, generator,
                                self.dis_model, is_generate, self.truncated_factor, self.prior, self.latent_op, self.latent_op_step,
                                self.latent_op_alpha, self.latent_op_beta, self.rank, self.cfgs.log_output_path, cate, self.step_count, index_to_class) 

                save_images_png(self.run_name, self.eval_dataloader, 10000, self.num_classes, self.enc_model, self.ali_model, generator,
                                self.dis_model, is_generate, self.truncated_factor, self.prior, self.latent_op, self.latent_op_step,
                                self.latent_op_alpha, self.latent_op_beta, self.rank, self.cfgs.log_output_path) 
            if npz:
    ...
    def run_frequency_analysis(self, num_images, standing_statistics, standing_step):
        ...
        with torch.no_grad() if self.latent_op is False else dummy_context_mgr() as mpc:
            ...
            plot_spectrum_image(real_gray_spectrum, fake_gray_spectrum, self.run_name, self.logger, self.cfgs.log_output_path)
    ...
    def run_tsne(self, dataloader, standing_statistics, standing_step):
        ...
        with torch.no_grad() if self.latent_op is False else dummy_context_mgr() as mpc:
            ...
            plot_tsne_scatter_plot(real, real_tsne_results, "real", self.run_name, self.logger, self.cfgs.log_output_path)

            fake_tsne_results = tsne.fit_transform(fake["embeds"])
            plot_tsne_scatter_plot(fake, fake_tsne_results, "fake", self.run_name, self.logger, self.cfgs.log_output_path)
