...
def plot_pr_curve(precision, recall, run_name, logger, log=False, log_output_path='./'):
    directory = join('%s/figures'%(log_output_path), run_name)
...
def plot_spectrum_image(real_spectrum, fake_spectrum, run_name, logger, log=False, log_output_path='./'):
    directory = join('%s/figures'%(log_output_path), run_name)
...
def plot_tsne_scatter_plot(df, tsne_results, flag, run_name, logger, log_output_path='./'):
    directory = join('%s/figures'%(log_output_path), run_name, flag)
...
def plot_sim_heatmap(similarity, xlabels, ylabels, run_name, logger, log=False, log_output_path='./'):
    directory = join('%s/figures'%(log_output_path), run_name)
        ...
        for i in tqdm(range(0, n_batches), disable=False):
            ...
            if is_generate:
                ...
                try:
                    ...
                    feat_enc = encoder(images, None, mode='enc')
                    images, labels = generate_images(batch_size, feat_enc, generator, discriminator, truncated_factor, prior, latent_op,
                                             latent_op_step, latent_op_alpha, latent_op_beta,  device)
                except StopIteration:
                    break
...
def save_all_cate_images_png(run_name, data_loader, num_samples, num_classes, encoder, aligner,  generator, discriminator, is_generate,
                    truncated_factor,  prior, latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, pathdir='.', refer_labels=None, step_count=0, index_to_class=None):
    if is_generate is True:
        batch_size = data_loader.batch_size
        n_batches = math.ceil(float(num_samples) / float(batch_size))
    else:
    ...
    directory = join('%s/samples_all_cates/%d'%(pathdir, step_count), run_name, type, "png")
    if not exists(abspath(directory)):
        os.makedirs(directory)
        for f in range(num_classes):
            os.makedirs(join(directory, index_to_class[f]))
    ...
    with torch.no_grad() if latent_op is False else dummy_context_mgr() as mpc:
        for i in tqdm(range(0, n_batches), disable=False):
            ...
            if is_generate:
                ...
                try:
                    images_real, labels = next(data_iter)
                    batch_size = len(images_real)
                    images_real, labels = images_real.to(device), labels.to(device)
                    feat_enc = encoder(images_real, None, mode='enc')
                    feat_enc = aligner(feat_enc)
                    images, labels = generate_images(batch_size, feat_enc, generator, discriminator, truncated_factor, prior, latent_op,
                                             latent_op_step, latent_op_alpha, latent_op_beta,  device, refer_labels)
            ...
            for idx, img in enumerate(images.detach()):
                if batch_size*i + idx < num_samples:
                    cate_name = index_to_class[labels[idx].item()]
                    save_image((img+1)/2, join(directory, cate_name , '{idx}.png'.format(idx=batch_size*i + idx)))
                else:
                    pass
            if refer_labels==0:
                for idx, img in enumerate(images_real.detach()):
                    gt_dir = '%s/samples_all_cates/GT'%(pathdir)
                    if not os.path.exists(gt_dir):
                        os.makedirs(gt_dir)
                    if batch_size*i + idx < num_samples:
                        save_image((img+1)/2, join(gt_dir, '{idx}.png'.format(idx=batch_size*i + idx)))
    print('Save png to %s/generated_images/%s' %(pathdir, run_name))
def save_images_png(run_name, data_loader, num_samples, num_classes, encoder, aligner, generator, discriminator, is_generate,
                    truncated_factor,  prior, latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, pathdir='.'):
    if is_generate is True:
        batch_size = data_loader.batch_size
        n_batches = math.ceil(float(num_samples) / float(batch_size))
    else:
    ...
    directory = join('%s/samples'%(pathdir), run_name, type, "png")
    if exists(abspath(directory)):
        shutil.rmtree(abspath(directory))
    os.makedirs(directory)
    for f in range(num_classes):
...
def Rec_loss(feat_source, feat_fake, rec_w={4:10, 8:5, 16:2, 32:0.1}):
  loss_M = 0.
  for keys in feat_source.keys(): 
       loss_M += rec_w['%d'%keys] * torch.mean(F.mse_loss(feat_source[keys], feat_fake[keys])) 
  return loss_M
