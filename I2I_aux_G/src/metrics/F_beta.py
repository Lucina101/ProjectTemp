...
class precision_recall(object):
    ...
    def generate_images(self, feat_enc, gen, dis, truncated_factor, prior, latent_op, latent_op_step, latent_op_alpha, latent_op_beta, batch_size):
    ...
    def compute_precision_recall(self, dataloader, gen, dis, num_generate, num_runs, num_clusters, truncated_factor, prior,
                                 latent_op, latent_op_step, latent_op_alpha, latent_op_beta, batch_size, device, num_angles=1001, condition_input=False,  encoder=None, aligner=None):
        ...
        for i in tqdm(range(n_batches), disable = self.disable_tqdm):
            ...
            feat_enc = encoder(real_images, None, mode='enc')
            feat_enc = aligner(feat_enc)
            fake_images = self.generate_images(feat_enc, gen, dis, truncated_factor, prior, latent_op, latent_op_step,
...
def calculate_f_beta_score(dataloader, gen, dis, inception_model, num_generate, num_runs, num_clusters, beta, truncated_factor,
                           prior, latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, logger, condition_input=False, encoder=None, aligner=None):
    ...
    precision, recall = PR.compute_precision_recall(dataloader, gen, dis, num_generate, num_runs, num_clusters, truncated_factor,
                                                    prior, latent_op, latent_op_step, latent_op_alpha, latent_op_beta, batch_size, device, condition_input=condition_input,  encoder=encoder, aligner=aligner)
