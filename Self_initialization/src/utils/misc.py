...
def save_all_cate_images_png(run_name, data_loader, num_samples, num_classes, encoder, aligner,  generator, discriminator, is_generate,
                    truncated_factor,  prior, latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, pathdir='.', refer_labels=None, step_count=0):
    if is_generate is True:
        batch_size = data_loader.batch_size
        n_batches = math.ceil(float(num_samples) / float(batch_size))
    else:
...
def Rec_loss(feat_source, feat_fake, rec_w={4:10, 8:5, 16:2, 32:0.1}):
  for idx, keys in enumerate(feat_source.keys()): 
       if idx ==0:
