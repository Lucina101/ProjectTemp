...
def calculate_accuracy(dataloader, generator, discriminator, D_loss, num_evaluate, truncated_factor, prior, latent_op,
                       latent_op_step, latent_op_alpha, latent_op_beta, device, cr, logger, eval_generated_sample=False,  condition_input=False, encoder=None, aligner=None):
    ...
    if eval_generated_sample:
        for batch_id in tqdm(range(total_batch), disable=disable_tqdm):
            ...
            fake_images = generator(zs, fake_labels, evaluation=True, feat_enc=feat_enc)
