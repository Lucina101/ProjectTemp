...
def generate_images(batch_size, feat_enc, gen, dis, truncated_factor, prior, latent_op, latent_op_step,
                    latent_op_alpha, latent_op_beta, device, refer_labels=None):
    ...
    if refer_labels is not None:
        fake_labels = torch.ones_like(fake_labels) * refer_labels
    ...
    with torch.no_grad():
        batch_images = gen(zs, fake_labels, evaluation=True, feat_enc=feat_enc)
...
def get_activations(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor, prior, is_generate,
                    latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, tqdm_disable=False, run_name=None, condition_input=False, encoder=None, aligner=None):
...
def calculate_activation_statistics(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor, prior,
                                    is_generate, latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, tqdm_disable, run_name=None,  condition_input=False,  encoder=None, aligner=None):
    act = get_activations(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor, prior,
                          is_generate, latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, tqdm_disable, run_name, condition_input, encoder, aligner)
...
def calculate_fid_score(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor, prior,
                        latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, logger, pre_cal_mean=None, pre_cal_std=None, run_name=None, condition_input=False, encoder=None, aligner=None):
    ...
    m2, s2 = calculate_activation_statistics(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor, prior,
                                             False, latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, tqdm_disable=disable_tqdm, run_name=run_name, condition_input=condition_input, encoder=encoder, aligner=aligner) # is_generate is False,since we need source images
