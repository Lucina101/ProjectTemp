...
class evaluator(object):
    ...
    def eval_gen(self, gen, dis, n_eval, truncated_factor, prior, latent_op, latent_op_step, latent_op_alpha,
                 latent_op_beta, split, batch_size, device, condition_input=False, encoder=None, data_loader=None, aligner=None):
        ys = []
        ...
        if not  condition_input:
            ...
            for i in tqdm(range(0, n_batches), disable=self.disable_tqdm):
                try:
                ...
                batch_images = self.generate_images(feat_enc, gen, dis, truncated_factor, prior, latent_op, latent_op_step,
                                                    latent_op_alpha, latent_op_beta, batch_size)
                y = self.inception_softmax(batch_images)
                ys.append(y)
...
def calculate_incep_score(dataloader, generator, discriminator, inception_model, n_generate, truncated_factor, prior,
                          latent_op, latent_op_step, latent_op_alpha, latent_op_beta, splits, device, logger, condition_input=False, encoder=None, aligner=None):
    ...
    kl_score, kl_std = evaluator_instance.eval_gen(generator, discriminator, n_generate, truncated_factor, prior, latent_op,
                                                   latent_op_step, latent_op_alpha, latent_op_beta, splits, batch_size, device, condition_input, encoder, dataloader, aligner)
