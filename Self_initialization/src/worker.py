...
class make_worker(object):
    def __init__(self, cfgs, run_name, best_step, logger, writer, n_gpus, gen_model, enc_model, ali_model,  dis_model, inception_model, Gen_copy,
                 Gen_ema, train_dataset, eval_dataset, train_dataloader, eval_dataloader, G_optimizer, A_optimizer, D_optimizer, G_loss,
    ...
    def train(self, current_step, total_step):
        ...
        while step_count <= total_step:
            ...
            for step_index in range(self.g_steps_per_iter):
                ...
                for acml_step in range(self.accumulation_steps):
                    with torch.cuda.amp.autocast() if self.mixed_precision else dummy_context_mgr() as mpc:
                        ...
                        #feat_enc = self.enc_model(source_images, None, mode='enc')
                        #feat_enc = self.ali_model(feat_enc)
                        fake_images, feat_gen = self.gen_model(zs, fake_labels, feat_enc=None, use_feat=False, output_feat_gen=True)
                        feat_enc = self.enc_model(fake_images, None, mode='enc')
                        feat_enc = self.ali_model(feat_enc)
                        ...
                        # reconstu
                        gen_acml_loss = Rec_loss(feat_gen, feat_enc, self.gen_model.rec_w)
                        #gen_acml_loss += 1000*rec_loss
               ...
               #     self.G_optimizer.step()
                ...
                step_count += 1
                print('Rec_loss: %f'%(gen_acml_loss.item()))
            ...
            if step_count % self.print_every == 0 and self.rank == 0:
                ...
                self.writer.add_scalars('Losses', {'discriminator': 0,
                                                   'generator': 0, 'rec_loss': gen_acml_loss.item(),}, step_count)
                if self.ada:
            ...
            if step_count % self.save_every == 0 or step_count == total_step:
                if self.evaluate:
                   ...
                   # self.save_images(False, False,  "N/A", png=True, npz=False, All_classes=False)#  :is_generate is False, since we perform I2I and need input source images.
    ...
    def save(self, step, is_best):
        ...
        if len(glob.glob(join(self.checkpoint_dir,"model=G-{when}-weights-step*.pth".format(when=when)))) >= 1:
            ...
            if  len(glob.glob(join(self.checkpoint_dir,"model=D-{when}-weights-step*.pth".format(when=when)))) > 0:
        ...
        if when == "best":
            if len(glob.glob(join(self.checkpoint_dir,"model=G-current-weights-step*.pth"))) >= 1:
                ...
                if len(glob.glob(join(self.checkpoint_dir,"model=D-current-weights-step*.pth"))) > 0:
            ...
            if png:
                ...
                if All_classes:
                    for cate in range(self.target_classes):
                        save_all_cate_images_png(self.run_name, self.eval_dataloader, 10, self.num_classes, self.enc_model, self.ali_model, generator,
                                self.dis_model, is_generate, self.truncated_factor, self.prior, self.latent_op, self.latent_op_step,
                                self.latent_op_alpha, self.latent_op_beta, self.rank, self.cfgs.log_output_path, cate, self.step_count) 
